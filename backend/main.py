...
def get_news(q: str = Query(..., description="Termo de busca")):
    ...
    try:
        with DDGS() as ddgs:
            results = list(ddgs.text(f"{q} Distrito Federal", region="br-pt", safesearch="off", max_results=10))
            if results:
                for r in results:
    ...
    if items:
        ...
        if REDIS_AVAILABLE and redis_client:
            try:
                redis_client.setex(cache_key, 600, json.dumps([i.dict() for i in items], default=str))
            except Exception as e:
